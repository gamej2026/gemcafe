...
namespace GemCafe.EditorTools
{
    public static class GemCafeSceneBuilder
    {
        ...
        private const string ResRockPath = "Assets/Resource/Ingredient_0.png";     // ?èå
        private const string ResGinsengPath = "Assets/Resource/Ingredient_1.png";  // ?ù∏?Çº
        private const string ResPersimmonPath = "Assets/Resource/Ingredient_2.png"; // Í≥∂Í∞ê
        ...
        public static void CreateSampleData()
        {
            ...
            EnsureSpriteImports();
            var sprWaterIcon = AssetDatabase.LoadAssetAtPath<Sprite>(ResPersimmonPath); // Í≥∂Í∞ê
            var sprSyrupIcon = AssetDatabase.LoadAssetAtPath<Sprite>(ResRockPath);       // ?èå
            var sprToppingIcon = AssetDatabase.LoadAssetAtPath<Sprite>(ResGinsengPath);  // ?ù∏?Çº
            ...
            cstDay1.orderDialogue = new[]
            {
                ...
                {
                    speakerId = "?Üê?ãò",
                    text = "?Çº?èÑÏ≤? Î¨ºÏóê ?ãú?üΩ ?Ñ£?ñ¥Ï£ºÍ≤å.",
                    portrait = null
                }
            };
            ...
            cstDay3.orderDialogue = new[]
            {
                ...
                {
                    speakerId = "?Üê?ãò",
                    text = "?ãú?üΩ??? ÎπºÍ≥† Î¨ºÍ≥º Í≥†Î™Ö?úºÎ°? Î∂??ÉÅ?ïò?Ñ§.",
                    portrait = null
                }
            };
        ...
        public static void BuildCafeScene()
        {
            ...
            ApplyDefaultText(speakerNameText, "?Üê?ãò", 28, TextAnchor.UpperLeft, Color.white);
            ...
            ApplyDefaultText(nextText, "?ã§?ùå", 24, TextAnchor.MiddleCenter, Color.white);
            ...
            SetString(speakerView, "leftSpeakerId", "Ï£ºÏù∏Í≥?");
            ...
            // ?ö∞?ÉÅ?ã® ?ä∏?†à?ù¥ (?Öå?ù¥Î∏? ?ÉëÎ∑?) ??? Tray.png
            ...
            // ?ä∏?†à?ù¥ ?úÑ ?û¨Î£? 3Ï¢?: Í≥∂Í∞ê(Ï¢åÏÉÅ) / ?èå(Ï¢åÌïò) / ?ù∏?Çº(?ö∞)
            ...
            // ?ö∞?ïò?ã® Ïª?(?Ç¨Î∞?) ??? ?û¨Î£? ?ìúÎ°? ???Íπ? "?ùåÎ£? Î≥¥Ïù¥?äî Í≥? (Ïª?)"
            ...
            ApplyDefaultText(bowlLabel, "?ùåÎ£? Î≥¥Ïù¥?äî Í≥? (Ïª?)", 40, TextAnchor.MiddleCenter, Color.white);
            bowlLabel.raycastTarget = false;

            // ÎßâÏûê (?ÑûÍ∏? ?èÑÍµ?) ??? ÏªµÏóê ?ìúÎ°??ïòÎ©? ?†úÏ°? ?ôÑÎ£?
            ...
            ApplyDefaultText(teawareGuideText, "?ã§Í∏∞Î?? ?àÑÎ•¥ÏÑ∏?öî", 24, TextAnchor.MiddleCenter, Color.white);
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                ApplyDefaultText(closeText, "?ã´Í∏?", 24, TextAnchor.MiddleCenter, Color.white);
            ...
            ApplyDefaultText(coinGainNextText, "?ã§?ùå", 24, TextAnchor.MiddleCenter, Color.white);
            ...
            ApplyDefaultText(endingCoinNextText, "?ã§?ùå", 24, TextAnchor.MiddleCenter, Color.white);
        ...
        public static void BuildLobbyScene()
        {
            ...
            ApplyDefaultText(titleText, "?Çº?èÑÏ≤? ?ã§Î∞?", 64, TextAnchor.MiddleCenter, Color.white);
            ...
            ApplyDefaultText(newGameText, "?Éà Í≤åÏûÑ", 24, TextAnchor.MiddleCenter, Color.white);
            ...
            ApplyDefaultText(continueText, "?ù¥?ñ¥?ïòÍ∏?", 24, TextAnchor.MiddleCenter, Color.white);
            ...
            ApplyDefaultText(settingsText, "?Ñ§?†ï", 24, TextAnchor.MiddleCenter, Color.white);
            ...
            ApplyDefaultText(quitText, "Í≤åÏûÑ Ï¢ÖÎ£å", 24, TextAnchor.MiddleCenter, Color.white);
        ...
        public static void BuildStage1Scene()
        {
            ...
            SetString(dolsoeInteractable, "displayName", "?èå?á†");
            SetDialogueLines(dolsoeInteractable, "dialogue", new[]
            {
                ("?èå?á†", "?ù¥Î≥¥Í≤å, ?Çº?èÑÏ≤úÏùÑ Í±¥ÎÑà?†§?äîÍ∞??"),
                ("Ï£ºÏù∏Í≥?", "...?Ñ§."),
                ("?èå?á†", "??? ?úÑ ?ã§Î∞©Ïóê ÎßàÎãò?ùÑ Ï∞æÏïÑÍ∞? Î≥¥Í≤å.")
            });
            ...
            SetString(manimInteractable, "displayName", "ÎßàÎãò");
            SetDialogueLines(manimInteractable, "dialogue", new[]
            {
                ("ÎßàÎãò", "?ñ¥?Ñú ?ò§Í≤?. ?ùº?Üê?ù¥ ?ïÑ?öî?ïò?çò Ï∞∏Ïù¥?ïº."),
                ("Ï£ºÏù∏Í≥?", "?†úÍ∞? ?èïÍ≤†Ïäµ?ãà?ã§."),
                ("ÎßàÎãò", "Ï¢ãÏïÑ, ?ïà?úºÎ°? ?ì§?ñ¥?ò§Í≤?.")
            });
            ...
            // Í∑ºÏ†ë ?ãú NPC ?Öå?ëêÎ¶? Î∞úÍ¥ë(?ïÑ?õÉ?ùº?ù∏ ?Ö∞?ù¥?çî) ?ó∞Í≤?.
        ...
        private static void ApplyDefaultText(Text text, string content, int fontSize, TextAnchor alignment, Color color)
        {
            ...
            text.color = color;
            // WebGLø°¥¬ OS ∆˘∆Æ ∆˙πÈ¿Ã æ¯æÓ ≥ª¿Â Arial∑Œ¥¬ «—±€¿Ã ∫∏¿Ã¡ˆ æ ¿∏π«∑Œ
            // ¿”∫£µÂµ» «—±€ ∆˘∆Æ∏¶ ¡˜¡¢ ∫£¿Ã≈©«—¥Ÿ. ∆˘∆Æ∞° æ¯¿∏∏È ≥ª¿Â ∆˘∆Æ∑Œ ∆˙πÈ.
