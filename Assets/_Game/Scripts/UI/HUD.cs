...
namespace GemCafe.UI
{
    public class HUD : MonoBehaviour
    {
        ...
        private void RefreshLives()
        {
            ...
            {
                // GameManagerÍ∞? ?ïÑÏß? Ï¥àÍ∏∞?ôî?êòÏß? ?ïä?ïò?úºÎ©? ?ïÑ?ù¥ÏΩòÏùÑ ?ÅÑÏß? ?ïäÍ≥?
                // Start ?ãú?†ê?óê ?ã§?ãú Í∞±Ïã†?ïú?ã§. (early OnEnableÎ°? ?ù∏?ïú ?ò§?îÑ Î∞©Ï??)
                return;
            }
        ...
        private void HandleCoinSlots(IReadOnlyList<CoinType> coins)
        {
            ...
            for (int i = 0; i < coinSlots.Length; i++)
            {
                ...
                if (coins != null && i < coins.Count)
                {
                    ...
                    var isGold = coinType == CoinType.Gold;
